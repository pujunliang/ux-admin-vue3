<template>
    <component
        ...
        v-model.trim="_searchParam[(<ColumnProps>column).search?.key ?? handleProp((<ColumnProps>column).prop!)]"
        :data="(<ColumnProps>column).search?.el === 'tree-select' ? columnEnum : []"
        :options="['cascader', 'select-v2'].includes((<ColumnProps>column).search?.el!) ? columnEnum : []">
        <template
            v-if="(<ColumnProps>column).search?.el === 'cascader'"
            #default="{ data }">
            <span>{{ data[fieldNames.label] }}</span>
        </template>
        <template v-if="(<ColumnProps>column).search?.el === 'select'">
            <component
                :is="`el-option`"
